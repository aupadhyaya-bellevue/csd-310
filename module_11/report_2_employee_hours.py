...
def employee_yearly_hours_report():
    ...
    employee_ids = mycursor.fetchall()
    ...
    while x < len(employee_ids):
        # Used an INNER JOIN to grab the needed columns of one employee per iteration of the while loop, with the y
        # variable allowing for iteration through the employees
        mycursor.execute(
            "SELECT first_name, last_name, department, hours_worked FROM employees e INNER JOIN hours_worked w ON w.employee_id = e.employee_id WHERE e.employee_id =" + str(
                employee_ids[x][0]))
        employee_info = mycursor.fetchall()
        # Appending each employees first name, last name, department and the sum of their previous four quartes worth
        # of hours
        employee_yearly.append([employee_info[0][0], employee_info[0][1], employee_info[0][2],
                                employee_info[0][3] + employee_info[1][3] + employee_info[2][3] + employee_info[3][3]])
        x = x + 1
        y = y + 1
